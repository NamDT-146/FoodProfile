...
class FoodProfile(Account):
    ...
    origin_u8_32_array: Array[u8, 32]  # 32 bytes (Public key of the creator as an array of u8)
...
def create_food_profile(
    ...
    transport_log_coordinate_long: f64,
    transport_log_time: i32,
    origin_u8_32_array: Array[u8, 32],  # Added origin field
    seed_random: u128
):
    food_profile = food_profile.init(
        payer=payer,
        seeds=[owner.key(), "food_profile", seed_random],
        space=32 + 32 + 64 + 4 + 32 + 4 + 4 + (8 + 8 + 4)  # Adjusted space for the new origin field
    )

    food_profile.owner = owner.key()
    food_profile.origin_u8_32_array = origin_u8_32_array  # Set the origin from the input
    ...
    # Initialize the transport log with the given entries
...
def get_food_profile(food_profile: FoodProfile):
    ...
    print(f'Origin: {food_profile.origin_u8_32_array}')  # Convert byte array to public key format
